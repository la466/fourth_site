...
# Script number: 			13.3
# File: 					3 of 3
# Prerequisite script(s):	1.1, 1.2, 2.1, 3.1, 3.2, 13.1, 13.2
# Prerequisite file(s):		_filteredGenes.csv, table4genomes.txt, _site_4_ratios.csv, dG file outputs
# Description: 				Analyse the dG scores calculated to identify the SD sequences
# Output file(s):			_sd_distances.csv, _sdSequenceAnalyis.csv

import sys, os, re, shutil, Bio, csv, random, subprocess, urllib, collections, numpy
...
cds_direc = "genome_extractions/cds/"
...
def get_rRNA_tail(acc,genome,fileNumber,numFiles, filteredGenes, maxTailsShort, maxTailsLong, antiSDfile):
...
def setup_free2bind(genome, antiSDsequences, genomeNum, filteredGenes,upstreamDirectory):
	...
	relPathToLaunch =  currentDir + "/packages/free2bind/"
...
def get_gene_sequences_for_dg(genome, filteredGenes):
	...
	geneFilePath = cds_direc + genome + ".txt"
...
def analyse_dgs(dGvalues, upstreamDirectory, GCs, GC3s, Aratios, antiSDsequences):
	...
	for genome in dGvalues:
		...
		if genome in os.listdir(upstreamDirectory):
			...
			if len(bind) != 0 or len(noBind) != 0:
				...
				fileLine = "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n" % (genome, GCs[genome], GC3s[genome], Aratios[genome], len(dGvalues[genome]), prop_sd, prop_no_sd, prop_a_sd, prop_a_no_sd, prop_weak_sd_a, prop_strong_sd_a,near_prop,far_prop)
	...
	outputFilePath = output_sd_dir + "_sd_distances.csv"
...
def get_genome_info(fourthPath):
...
def main():
	...
	# Retrieve the information about the genomes
	fourthPath = outputDir + "/ratio_testing/site_4/_site_4_ratios.csv"
	GCs, GC3s, Aratios = get_genome_info(fourthPath)
	...
	# 			# if acc in testFile:
	# 				fileNumber += 1
	# 				get_rRNA_tail(acc,genome,fileNumber,numFiles, filteredGenes, maxTailsShort, maxTailsLong, antiSDfile)
	...
	upstreamDirectory = output_sd_dir + "/antiSD_upstream_seq/"
	...
	analyse_dgs(dGvalues, upstreamDirectory, GCs, GC3s, Aratios, antiSDsequences)
