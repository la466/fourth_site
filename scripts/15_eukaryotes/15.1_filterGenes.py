...
# Prerequisite file(s):		genome fasta file
...
#								Filter 5: Gene must use an ATG start codon
# Output file(s):			_filteredGenes_eukaryotes.csv, _badGenes_eukaryotes.csv, _startCodons_eukaryotes.csv, table4genomes_eukaryotes.txt
...
print "\nFiltering the genes in the genome\n"
...
newDir = output_directory + "/gene_filtering/"


# Define the stop codons for the translation tables
translation_tables = {}
translation_tables["table1"] = ["TAG", "TAA", "TGA"]
...
def newFile():
	...
	filteredFile = newDir + "_filteredGenes_eukaryotes.csv"
	filtered = open(filteredFile, "w")

	# Output file containing the genes failing the filter
	badFile = newDir + "_badGenes_eukaryotes.csv"
	bad = open(badFile, "w")

	# Output file containing the genes failing the filter
	startCodonFilePath = newDir + "_startCodons_eukaryotes.csv"
	startCodonFile = open(startCodonFilePath, "w")



	return filtered, bad, startCodonFile


# Set up the file heading for the good genes
def fileHeadFiltered(file):
	filteredHead = "Genome,Acc,Gene\n"
...
def filterGenes(genome,genes,fileNumber,numGenomes,filtered,bad):
	...
	trans_table1 = False
	genome_path = genomesDir + genome + ".fa"
	print genome_path
...
def main():
	...
	# Open the new files
	filtered,bad,startCodonFile = newFile()
	...
	for genome in genomes:
		fileNumber +=1
		file = genomesDir + genome + ".fa"
		genes = splitFile(file)
		filterGenes(genome,genes,fileNumber,numGenomes,filtered,bad)
