...
# Output file(s):			_stopDistanceGenome.csv, _stopDistanceTable4Genome.csv
...
def newFile():
	...
	stopDistanceDistFile = stop_directory + "_stopDistanceGenome.csv"
	stopDistanceDist = open(stopDistanceDistFile, "w")


	stopDistanceTable4DistributionFile = stop_directory + "_stopDistanceTable4Genome.csv"
...
def getStopDistance(genomes,acc,numGenomes,fileNumber, stopDistance,stopDistanceTable4):
	...
	with open(genomeDir) as myfile:
		...
		propNoStops = {}
		propNoStops["A"] = proportion(noStops["A"], numberAgenes)
		propNoStops["notA"] = proportion(noStops["notA"], numberNotAgenes)

		meanStops = {}
		meanStops["A"] = proportion(numberStops["A"], numberAgenes)
		meanStops["notA"] = proportion(numberStops["notA"], numberNotAgenes)


		genomeLine += ",%s,%s,%s,%s,%s,%s,%s" % (meanDistance["all"], meanDistance["A"], meanDistance["notA"], propNoStops["A"], propNoStops["notA"], meanStops["A"], meanStops["notA"])
		genomeLine += "\n"
