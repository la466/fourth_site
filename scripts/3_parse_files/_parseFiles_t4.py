...
# Script number: 			3.3
# File: 					3 of 3
# Prerequisite script(s):	1.1, 1.2, 2.1
# Prerequisite file(s):		raw embl genome files (main directory)
# Description: 				Parse the files for all of the table 4 genomes in the t4_genomes folder
...
outDir = "genome_extractions/"
newDir = "genome_extractions/t4/"

# Set the directory with the raw good files
goodDir = 'bacterial_genomes/t4_genomes/'
...
def parseFile(genome,fileNumber,numFiles):
	...
	genus = getGenus(genome)

	output = [genome[:-5], genus, record.description]
...
def main():
	...
	# Set up the new directories
	setupNiceDirectories(outDir)
	...
	for genome in goodFiles:
		...
		fileNumber += 1
		output = parseFile(genome,fileNumber,numFiles)
