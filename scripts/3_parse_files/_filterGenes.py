...
# Script number: 			3.2
# File: 					2 of 3
# Prerequisite script(s):	1.1, 1.2, 2.1, 3.1
# Prerequisite file(s):		Extracted coding sequences
...
def newFile():
	...
	filteredFile = newDir + "_filteredGenes.csv"
	filtered = open(filteredFile, "w")

	# Output file containing the genes failing the filter
	badFile = newDir + "_badGenes.csv"
	bad = open(badFile, "w")

	# Output file containing the genes failing the filter
	startCodonFilePath = newDir + "_startCodons.csv"
	startCodonFile = open(startCodonFilePath, "w")

	table4genomesPath = newDir + "table4genomes.txt"
